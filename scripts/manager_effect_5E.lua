...
function onEffectAddIgnoreCheck(nodeCT, rEffect)
	...
	if #aCancelled > 0 then
		if rEffect.sName == "" then
			return "Effect ['" .. rEffect.sName .. "'] -> [TARGET IMMUNE]";
		else
...
function evalAbilityHelper(rActor, sEffectAbility)
	...
	elseif sShortAbility == "BLO" then
		nAbility = ActorManager2.getAbilityBonus(rActor, "bloodline");
	elseif sShortAbility == "HON" then
		nAbility = ActorManager2.getAbilityBonus(rActor, "honor");
